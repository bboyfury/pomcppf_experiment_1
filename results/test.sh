...
TRAJECTORIES_LIST=(10 50 100 200 300 500)
HORIZON_LIST=(5 10 20 30)
PARTICLES_LIST=(5 10 20 30 50)
...
for TRAJECTORIES in "${TRAJECTORIES_LIST[@]}"; do
  for HORIZON in "${HORIZON_LIST[@]}"; do
    for PARTICLES in "${PARTICLES_LIST[@]}"; do
      ...
      sbatch test.slurm $SETUP $START_RUN $STOP_RUN $EPSILON $TRAJECTORIES $HORIZON $UCB_C $PARTICLES
    done
  done
done
